...
from tensorflow.keras.layers import Layer, Add, Permute, multiply, Lambda, Activation, Dense, Concatenate
...
class ChannelAttention(Layer):
    def __init__(self, in_planes, ratio=16):
        ...
        self.convolution1 = Conv2D(in_planes // ratio, kernel_size=1, strides=1, padding='same',
                                   kernel_regularizer=regularizers.l2(5e-4),
                                   use_bias=True, activation=tf.nn.relu)
        self.convolution2 = Conv2D(in_planes, kernel_size=1, strides=1, padding='same',
                                   kernel_regularizer=regularizers.l2(5e-4),
                                   use_bias=True)
    ...
    def call(self, inputs, *args, **kwargs):
        ...
        avg_out = self.convolution2(self.convolution1(avg))
        max_out = self.convolution2(self.convolution1(max))
        out = avg_out + max_out
